...
from writes import write_to_gcs_stream
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("SparkConsumer-GCS").config("spark.jars.packages", "org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.3,io.delta:delta-spark_2.12:3.2.0").config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").getOrCreate()
    ...
    write_to_gcs_stream(stream_df, "streams")
    write_to_gcs_stream(stream_tags_df, "stream_tags")
    write_to_gcs_stream(user_df, "users")
    write_to_gcs_stream(video_df, "videos")
    write_to_gcs_stream(clip_df, "clips")
    write_to_gcs_stream(top_game_df, "top_games")
